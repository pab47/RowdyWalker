%derive2DOF_rear_stance_Sec3_3_Sec3_4
%Files used to derive equations given in Sec 3.3 and Sec 3.4 
clc;
clear all;
...
%Original position vectors (see Fig 6 b)
r_H_G1 = c2*X1 - w2*Y1; % m2 is moving leg. Here, this is the stance leg
r_H_G2 = c1*X2 - w1*Y2;  % m1 is body and front fixed foot. Here, this is the swing leg
r_P1_H = r*j;% - l*X1;
...
M_P1    = dot(m1*g*cross(r_P1_G2,-J)+m2*g*cross(r_P1_G1,-J),k);
Hdot_P1 = dot(I1*al2+m1*cross(r_P1_G2,a_G2)+I2*al1+m2*cross(r_P1_G1,a_G1),k);

M_H    = dot(m1*g*cross(r_H_G2,-J),k); 
Hdot_H = dot(I1*al2+m1*cross(r_H_G2,a_G2),k);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%    Angular Momentum, (Before and After Heelstrike  % 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
H_P1p = dot(I2*om1+m2*cross(r_P1_G1,v_G1)+I1*om2+m1*cross(r_P1_G2,v_G2),k);
H_P1n = subs(H_P1p,{u1,u2,q1,q2},{v1,v2,r1,r2});

AM_P1 = H_P1p;
Mh11  =  subs(AM_P1,[u1 u2],[1 0]);
RHSh1 = H_P1n;
disp('Copy paste in front_strike');
disp('-----------');
...
%%%% two bodies %%%%%%
   ...
   RHSs1 = RHSs1 + C1*u1; %added C1*u1
   
   
disp('Copy paste in single_stance2_rear_stance');
disp('-----------');
...
%KE_m2_temp = 0.5*simplify(m2*dot(v_G1,v_G1)+I2*u1*u1)
%KE_m1_temp = 0.5*simplify(m1*dot(v_G2,v_G2)+ I1*(u1-u2)*(u1-u2))
KE = 0.5*(simplify(I2*u1*u1+m2*dot(v_G1,v_G1) + I1*(u1-u2)*(u1-u2)+m1*dot(v_G2,v_G2)));
PE = simplify(m2*g*Y_G1+m1*g*Y_G2);
disp(' ');
disp('total_energy2_rear_stance');
TE = simplify(KE + PE); %substract -C1*u1 due to dissipation
%disp(TE);
